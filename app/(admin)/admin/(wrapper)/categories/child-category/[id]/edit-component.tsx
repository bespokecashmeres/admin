"use client";
import {
  ChildCategoryFormComponent,
  LoadingMessage
} from "@/components";
import adminAxiosInstance from "@/config/adminAxiosInstance";
import { MESSAGES } from "@/constants";
import { CHILD_CATEGORY_GET_URL} from "@/constants/apis";
...
const EditComponent = ({ id, genderData }: { id: string; genderData: DropDownOptionType[] }) => {
  ...
  return !loading && editData && genderData ? (
    <ChildCategoryFormComponent editData={editData} genderList={genderData} />
  ) : (
    <LoadingMessage />
  );
};
