...
import {
  ...
  SUB_CHILD_CATEGORY_ADD_URL,
  SUB_CHILD_CATEGORY_UPDATE_URL,
} from "@/constants/apis";
...
const SubChildCategoryFormComponent = ({
  editData,
  genderList,
}: {
  editData?: any;
  ...
  const methods = useForm<SubChildCategoryFormType>({
    defaultValues: {
  ...
  useEffect(() => {
    if (editData) {
      const fetchData = async () => {
        ...
        const [mainCategoryRes, subCategoryRes, childCategoryRes] =
          await Promise.all([
        ...
        const childCategoriesData = childCategoryRes.data as any;
        ...
        if (childCategoriesData.success) {
          setChildCategoryList(
            childCategoriesData.data.map((category: any) => ({
              label: category.label,
              value: category.value,
              image: getAWSImageUrl(category.image),
            }))
  ...
  const onSubmit = async (data: SubChildCategoryFormType) => {
    try {
    ...
    <FormProvider {...methods}>
      <form onSubmit={methods.handleSubmit(onSubmit)}>
        <div className="space-y-4">
          ...
          <div className="grid gap-5 mt-4 sm:grid-cols-1 md:grid-cols-2">
            <RHFFormDropdownField
              name="genderId"
...
export default SubChildCategoryFormComponent;
